...
def resource_path(rel: str) -> _Path:
    return _Path(__file__).parent / rel
...
class ModelConfig:
    ...
    def __init__(
        ...
        self.MODEL_PATH = model_path if model_path is not None else resource_path("models/phomenizer_en.onnx")
        self.PHONEME_DICT_PATH = (
            phoneme_dict_path if phoneme_dict_path is not None else resource_path("models/lang_phoneme_dict.pkl")
        )
        self.TOKEN_TO_IDX_PATH = (
            token_to_idx_path if token_to_idx_path is not None else resource_path("models/token_to_idx.pkl")
        )
        self.IDX_TO_TOKEN_PATH = (
            idx_to_token_path if idx_to_token_path is not None else resource_path("models/idx_to_token.pkl")
        )
